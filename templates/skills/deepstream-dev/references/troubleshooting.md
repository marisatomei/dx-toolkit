...
> **Important**: `libmosquitto1` is the client _library_ only. If you also need to run an MQTT broker locally (e.g., `mosquitto &`) or use CLI tools like `mosquitto_sub` / `mosquitto_pub` for testing, you must install **separate** packages:
>
> ```bash
